...
class Dialogue : MonoBehaviour
{
    ...
    {
        if(_dialogueDBDatas == null || _dialogueDBDatas.Count == 0)
            _dialogueDBDatas = SpecDataManager.instance.DialogueDBDatas.FindAll(x => 1000 <= x.group_id && x.group_id < 2000);

        // 새로운 모험가 방문 -> 이전 모험가와의 대화 삭제
        if (dialogueType == 0) DeleteSpeechBubble();

        // 필요한 대화 그룹 로딩
        List<DialogueData> dialogueDatas = _dialogueDBDatas.FindAll(x => x.group_id == group_id && x.dialogue_type == dialogueType).ToList();
    ...
    /////////////////// private
    private List<DialogueData>          _dialogueDBDatas;                           // Dialogue DB
